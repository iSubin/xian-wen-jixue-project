...
4. If there is no active change, distinguish chat and direct non-executable maintenance from a governed task; only the latter routes to `xian-open`.
...
[批次衔接](references/authorized-batch.md)：仅在明确授权多 Change 批次时读取；普通单任务不预读批次细节。单一已授权任务仍按 current nextAction 连续完成，不增加固定审批。
...
For work already routed as a governed Change (not every document edit or development test):
