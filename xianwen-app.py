...
from src.main.python.xianwen.utils.logger import logger
from src.main.python.xianwen.db import TaskStatus
...
def ensure_proxy_env_for_startup() -> None:
    """启动时自动兜底代理环境，支持 python3 xianwen-app.py 直启。"""
...
def register_mdns_service(host: str, port: int):
    ...
    try:
        ...
        g_mdns_info = ServiceInfo(
            "_http._tcp.local.",
            "xianwen._http._tcp.local.",
            addresses=[socket.inet_aton(host)],
            port=port,
            properties={
                b"path": b"/",
                b"description": b"XianWen - Video Transcription Service",
            },
        )
        
        g_zeroconf.register_service(g_mdns_info)
        logger.info(f"--- [mDNS] 服务已注册: xianwen.local -> http://{host}:{port} ---")
        logger.info(f"--- [mDNS] 局域网设备可通过 'http://xianwen.local:{port}' 访问 ---")
...
async def run_sidebar_progress_test():
    ...
    # 1. 在数据库中创建一个假的测试任务
    from src.main.python.xianwen.db import db
    from src.main.python.xianwen.api import notify_task_update
...
async def lifespan(app: FastAPI):
    ...
    async def _prewarm_workers():
        try:
            ...
            from src.main.python.xianwen.llm.llm import get_llm
...
if __name__ == "__main__":
    ...
    logger.info(f"╔════════════════════════════════════════════════════════════╗")
    logger.info(f"║  先闻继学 XianWen v{APP_VERSION}                       ║")
    logger.info(f"╚════════════════════════════════════════════════════════════╝")
